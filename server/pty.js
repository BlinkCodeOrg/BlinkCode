...
// @ts-expect-error the native PTY package does not expose compatible declarations
...
export function createPtyManager({ getDefaultCwd }) {
  ...
  function create({ id, cwd, cols = 120, rows = 30, title, command }) {
  ...
  function resize(id, cols, rows) {
    ...
    try {
      ...
      // A short command can exit between the state check and the native resize call.
    }
  }
  ...
  function close(id) {
    ...
    if (!session) return;
    if (!session.exited) {
      if (session.hasCommand) {
        try { session.process.write('\x03'); } catch {}
