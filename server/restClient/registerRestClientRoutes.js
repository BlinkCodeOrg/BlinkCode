...
export function registerRestClientRoutes(app) {
  app.post('/api/rest-client/send', async (req, res) => {
    try {
      ...
      if (!request) return res.status(400).json({ error: 'HTTP request was not found' });
      ...
      const message = error?.name === 'AbortError' ? 'HTTP request timed out after 30 seconds' : error?.message;
