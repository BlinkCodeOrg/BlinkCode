...
export default function CodeEditor({ group = 'primary' }: { group?: 'primary' | 'secondary' }) {
  ...
  const isUnsupportedTextFile = getIsUnsupportedTextFile(activeFile, supportInfo, isSettingsJson);
  const blameInfo = useGitInlineBlame(activeFile, state.settings.gitInlineBlame);
  ...
  if (typeof activeFile.size === 'number' && activeFile.size > LARGE_FILE_LIMIT) {
    return (
      <LargeFilePreview
        file={activeFile}
        onUpdate={(content, offset, done) => dispatch({
