...
test('uses the BlinkCode debug configuration path and custom picker', async ({
  page,
}) => {
  ...
  await expect(page.getByTestId('debug-panel').locator('select')).toHaveCount(
    0,
  );
  await page.getByTitle('Create BlinkCode debug configuration').click();
  await expect(
    page.getByText('Created .blinkcode/launch.json', { exact: true }),
  ).toBeVisible();
});

test('evaluates primitives and objects in the paused stack frame', async ({
  page,
}) => {
...
test('shows evaluation errors and recovers on the next expression', async ({
  page,
}) => {
  await page.getByTestId('debug-start').click();
  await expect(page.locator('.debug-status')).toContainText('paused', {
  ...
  await input.press('Enter');
  await expect(page.locator('.debug-console-error')).toContainText(
