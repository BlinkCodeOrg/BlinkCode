...
test('extension runtime is permission-gated and excludes Node globals', async () => {
  const runtime = await readFile('server/extensions/activateExtension.js', 'utf8');
  assert.match(runtime, /vm\.createContext/);
  assert.match(runtime, /codeGeneration:\s*\{\s*strings:\s*false,\s*wasm:\s*false/);
...
test('extension marketplace packages ship with explicit manifests and entry files', async () => {
  const registry = JSON.parse(await readFile('extensions/marketplace/marketplace.json', 'utf8'));
  ...
  for (const extension of registry.extensions) {
    ...
    assert.equal((await readFile(`${root}/${manifest.main}`, 'utf8')).length > 0, true);
    assert.equal((await readFile(`${root}/${manifest.readme}`, 'utf8')).length > 0, true);
