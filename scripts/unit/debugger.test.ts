...
test('formats inspector call frames as workspace-relative locations', () => {
  ...
  const frames = formatCallFrames([{
    callFrameId: 'frame-1',
    functionName: 'run',
    url: new URL(`file:///${join(workspace, 'src/index.js').replace(/\\/g, '/')}`).href,
...
test('parses BlinkCode JSONC launch configurations and resolves workspace variables', () => {
  ...
  const configuration = resolveDebugConfiguration(document.configurations[0], workspace, 'src/index.js');
...
test('creates and loads only the BlinkCode-owned debug configuration path', async () => {
  ...
  try {
    ...
    assert.equal(existsSync(join(workspace, '.blinkcode', 'launch.json')), true);
...
test('Node debugger pauses at a breakpoint and exposes local variables', { timeout: 15_000 }, async () => {
  const workspace = mkdtempSync(join(tmpdir(), 'blinkcode-debug-'));
  writeFileSync(join(workspace, 'sample.js'), [
    'const value = 2;',
  ...
  try {
    ...
    while (
      Date.now() < deadline
      && !(session.snapshot().status === 'paused' && session.snapshot().callFrames[0]?.line === 3)
    ) {
    ...
    assert.equal(variables.find(variable => variable.name === 'value')?.value, '2');
    ...
    const objectProperties = await session.variables(objectEvaluation.objectId);
    assert.equal(objectProperties.find(variable => variable.name === 'answer')?.value, '42');
    assert.equal(objectProperties.find(variable => variable.name === 'label')?.value, 'BlinkCode');
    await assert.rejects(
      session.evaluate('missingBlinkCodeValue', frame.id),
      /missingBlinkCodeValue is not defined/,
    );
    await assert.rejects(session.evaluate('const =', frame.id), /SyntaxError|Unexpected token/);
    const consoleOutput = session.snapshot().output.filter(line => line.stream === 'console').map(line => line.text).join('');
...
test('launch configuration passes arguments and environment and can restart', { timeout: 20_000 }, async () => {
  const workspace = mkdtempSync(join(tmpdir(), 'blinkcode-debug-config-'));
  writeFileSync(join(workspace, 'configured.js'), [
  ...
  try {
    await session.startConfiguration(configuration, [{ path: 'configured.js', line: 1, enabled: true }]);
    const pausedDeadline = Date.now() + 8_000;
    while (session.snapshot().status !== 'paused' && Date.now() < pausedDeadline) {
      await new Promise(resolve => setTimeout(resolve, 50));
    }
    ...
    while (!session.snapshot().output.some(line => line.text.includes('environment:argument')) && Date.now() < outputDeadline) {
      await new Promise(resolve => setTimeout(resolve, 50));
    }
    assert.match(session.snapshot().output.map(line => line.text).join(''), /environment:argument/);
...
test('inspector URL resolver keeps direct WebSocket endpoints', async () => {
  assert.equal(await resolveInspectorUrl('ws://127.0.0.1:9229/session'), 'ws://127.0.0.1:9229/session');
});
...
test('attaches to an existing Node inspector and can pause it', { timeout: 15_000 }, async () => {
  ...
  try {
    const inspectorUrl = await new Promise<string>((resolve, reject) => {
      const timer = setTimeout(() => reject(new Error('Inspector URL timeout')), 8_000);
      child.stderr.on('data', chunk => {
        const match = String(chunk).match(/Debugger listening on (ws:\/\/[^\s]+)/);
        if (match) {
          clearTimeout(timer);
          resolve(match[1]);
        }
